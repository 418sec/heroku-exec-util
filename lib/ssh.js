...
function connect(context, tunnelHost, tunnelPort, clientUser, privateKey, callback) {
...
function ssh(context, tunnelHost, tunnelPort, clientUser, privateKey) {
...
function scp(context, tunnelHost, tunnelPort, clientUser, privateKey, src, dest) {
  return new Promise((resolve, reject) => {
    var conn = new Client();
    conn.on('ready', function() {
      cli.action.done('up')
