...
function connect(context, tunnelHost, clientUser, privateKey, callback) {
...
function ssh(context, clientUser, tunnelHost, privateKey) {
...
function scp(context, tunnelHost, clientUser, privateKey, src, dest) {
  return new Promise((resolve, reject) => {
    var conn = new Client();
    conn.on('ready', function() {
      cli.action.done('up')
